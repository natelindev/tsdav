...
test('davRequest should be able to send normal webdav requests', async () => {
  const [result] = await davRequest({
    url: process.env.CREDENTIAL_DAVICAL_SERVER_URL,
    init: {
      method: 'PROPFIND',
      headers: {
        ...authHeaders,
  ...
  expect(result.props?.currentUserPrincipal.href).toMatch(/\/caldav\.php\/.+/);
...
test('davRequest should be able to get raw xml response', async () => {
  ...
  expect(result.raw).toMatch(
    /<current-user-principal>\s+<href>\/caldav\.php\/.+\/<\/href>\s+<\/current-user-principal>/,
  );
});
