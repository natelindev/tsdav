...
test('calendarQuery should be able to query calendar objects', async () => {
  ...
  await deleteObject({ url: objectUrl, headers: authHeaders });
});
...
test('createCalendarObject should be able to create calendar object', async () => {
  ...
  const objects = await fetchCalendarObjects({
    calendar: calendars[0],
    objectUrls: [new URL('highlevel-create.ics', calendars[0].url).href],
    headers: authHeaders,
  });

  expect(objects.length).toBe(1);
  expect(objects[0].data.length > 0).toBe(true);

  await deleteObject({
    url: new URL('highlevel-create.ics', calendars[0].url).href,
    headers: authHeaders,
  });
});
...
test('updateCalendarObject should be able to update calendar object', async () => {
  const iCalString = await fsp.readFile(`${__dirname}/../data/ical/14.ics`, 'utf-8');
  const updatedICalString = await fsp.readFile(`${__dirname}/../data/ical/15.ics`, 'utf-8');
  const calendars = await fetchCalendars({
    account,
    headers: authHeaders,
  });
  ...
  const [obj] = await fetchCalendarObjects({
    calendar: calendars[0],
    objectUrls: [new URL('highlevel-update.ics', calendars[0].url).href],
    headers: authHeaders,
  });

  const updateResult = await updateCalendarObject({
    calendarObject: {
      url: new URL('highlevel-update.ics', calendars[0].url).href,
      etag: obj.etag,
...
test('deleteCalendarObject should be able to delete calendar object', async () => {
  ...
  const [obj] = await fetchCalendarObjects({
    calendar: calendars[0],
    objectUrls: [new URL('highlevel-delete.ics', calendars[0].url).href],
    headers: authHeaders,
  });

  const deleteResult = await deleteCalendarObject({
    calendarObject: {
      url: new URL('highlevel-delete.ics', calendars[0].url).href,
      etag: obj.etag,
    },
    headers: authHeaders,
  });
