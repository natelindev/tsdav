...
test('davRequest should be able to send normal webdav requests', async () => {
  const [result] = await davRequest({
    url: 'https://caldav.messagingengine.com/dav/calendars',
    init: {
      method: 'PROPFIND',
      headers: authHeaders,
      namespace: 'd',
  ...
  expect(result.props?.currentUserPrincipal.href).toMatch(/\/dav\/principals\/user\/.+\//);
  expect(Object.hasOwn(result, 'raw')).toBe(true);
});
...
test('davRequest should be able to get raw xml response', async () => {
  ...
  expect(result.raw).toMatch(
    /<d:current-user-principal>\s+<d:href>\/dav\/principals\/user\/.+\/<\/d:href>\s+<\/d:current-user-principal>/,
  );
});
